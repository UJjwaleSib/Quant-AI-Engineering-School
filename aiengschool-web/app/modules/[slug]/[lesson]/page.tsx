...
import {
  ...
  Lightbulb, Zap, BookOpen, Code2, ChevronRight as Next,
} from "lucide-react";
...
export default function LessonPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const lessonData = await api.curriculum.getLesson(slug, lessonIndex);
      setData(lessonData);
    } catch (err) {
  ...
  // Reset step state when lesson changes
  useEffect(() => {
    setStepIndex(0);
          ...
          <div className="flex items-center gap-2">
            {(["concept", "exercise"] as const).map((tab) => (
              ...
              >
                ...
                {tab === "concept" ? "Concept" : `Exercises (${data?.exercises.length ?? 0})`}
              </button>
            ))}
          </div>
          <Link
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {activeTab === "concept" && data && (
            <div className="max-w-3xl mx-auto px-8 py-8 space-y-6">
              ...
              {/* Why it matters — always at the top */}
              <button
              ...
              >
                ...
                <span className="text-sm font-medium text-amber-300 flex-1 text-left">Why This Matters</span>
              ...
              {/* ── STEP-BY-STEP LEARNING (new lessons) ── */}
              {hasSteps ? (
                <>
                  {/* Step progress bar */}
                    ...
                    {steps.map((s, i) => (
                      <button
                        ...
                        className={`h-1.5 rounded-full transition-all ${
                          i < stepIndex
                            ? "flex-1 bg-indigo-500 cursor-pointer"
                            : i === stepIndex
                            ? "flex-[2] bg-indigo-400"
                            : "flex-1 bg-[#2e3245] cursor-default"
                        }`}
                        title={i <= stepIndex ? s.title : "Complete previous steps first"}
                      />
                    ))}
                  </div>
                  <p className="text-xs text-slate-500 -mt-3">
                    Step {stepIndex + 1} of {steps.length}
                    {showFullExample ? " · Full example" : ""}
                  </p>

                  {/* Current step card */}
                  <div className="bg-[#1a1d27] border border-[#2e3245] rounded-xl overflow-hidden">
                    <div className="px-6 py-5 border-b border-[#2e3245]">
                    ...
                    <div className="px-6 py-5 space-y-4">
                      <p className="text-sm text-slate-300 leading-7">{currentStep.body}</p>
                      {currentStep.snippet && (
                        <div className="bg-[#0a0d14] border border-[#2e3245] rounded-lg overflow-hidden">
                          <div className="px-4 py-2 border-b border-[#2e3245] flex items-center gap-2">
                            ...
                            <span className="text-xs text-emerald-400 font-mono font-semibold uppercase tracking-wider">Code</span>
                          </div>
                          <pre className="p-4 overflow-x-auto text-xs leading-6 text-slate-300 font-mono">
                            {currentStep.snippet}
                          </pre>
                        </div>
                      )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {!isLastStep ? (
                      ...
                      >
                        Next Step <Next className="w-4 h-4" />
                      </button>
                    ) : !showFullExample ? (
                      <button
                        onClick={() => setShowFullExample(true)}
                        className="flex items-center gap-2 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 text-emerald-400 text-sm font-medium px-5 py-2.5 rounded-lg transition-all"
                      >
                        <Code2 className="w-4 h-4" /> See Full Example
                      </button>
                    ) : null}
                  </div>

                  {/* Full code example — revealed after all steps */}
                  {showFullExample && (
                    <div className="bg-[#1a1d27] border border-emerald-500/20 rounded-xl overflow-hidden">
                      <div className="px-5 py-3 border-b border-emerald-500/20 flex items-center gap-2">
                        <Code2 className="w-3.5 h-3.5 text-emerald-400" />
                        <span className="text-xs font-bold uppercase tracking-wider text-emerald-400">Full Worked Example</span>
                      </div>
                ...
                /* ── LEGACY LESSONS (no explanation_steps) ── */
                <>
              ...
              <div className="flex items-center gap-4 pt-2">
                ...
                >
                  <Zap className="w-4 h-4" /> Start Exercises ({data.exercises.length})
                </button>
              </div>
            </div>
          )}
